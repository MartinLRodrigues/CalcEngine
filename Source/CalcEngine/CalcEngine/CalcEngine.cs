...
namespace CalcEngine
{
	...
	public partial class CalcEngine
	{
		...
		Dictionary<string, Variable> _vars;       // table with variables
		...
		public CalcEngine()
		{
			...
			_vars = new Dictionary<string, Variable>(StringComparer.OrdinalIgnoreCase);
		...
		{
			RegisterFunction(functionName, parmCount, parmCount, fn, true);
		}
		/// <summary>
		...
		Dictionary<object, Token> GetSymbolTable()
		{
			if (_tkTbl == null)
			{
                ...
                AddToken('~', TKID.SUPPRESS, TKTYPE.ADDSUB);
		...
		Expression ParseCompare()
		{
			...
			while (_token.Type == TKTYPE.COMPARE)
			{
				...
				x = new BinaryExpression(_observer, t, x, exprArg);
			}
			return x;
		}
		...
		Expression ParseMulDiv()
		{
			...
			while (_token.Type == TKTYPE.MULDIV)
			{
				...
				GetToken();
				var a = ParsePower();
				x = new BinaryExpression(_observer, t, x, a);
			}
			return x;
		}
		...
		Expression ParseUnary()
		{
			...
			if (_token.ID == TKID.ADD || _token.ID == TKID.SUB || _token.ID == TKID.SUPPRESS)
			{
				var t = _token;
				GetToken();
				var a = ParseAtom();
				return new UnaryExpression(_observer, t, a);
			}
